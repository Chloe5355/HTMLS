...
flippedCards=[]; cards=[]; currentTurn='player';
...
// ゲーム開始
function startGame(){
  playerScore=0; aiScore=0; matchedPairs=0; flippedCards=[]; currentTurn='player';
  updateTurnDisplay();
  ...
  let temp=[]; for(let i=0;i<neededPairs;i++) temp.push(...cardEmojis);
  temp=temp.slice(0,totalPairs); cards=[]; temp.forEach(e=>{cards.push(e);cards.push(e);});
  shuffle(cards);
...
function renderBoard(){
  ...
  cards.forEach((v,i)=>{
    ...
    card.className='card';
    card.dataset.value=v; card.dataset.index=i;
    card.textContent='?';
...
function endGame(){
  ...
  let result;
  if(playerScore>aiScore){result='🎉 あなたの勝ち！'; currentEquippedTitle=titles[Math.floor(Math.random()*titles.length)];}
